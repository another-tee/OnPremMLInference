...
from converter.classification import build_engine

# --------------------------------------------------------------------------- #
#                         Define functions/classes                            #
# --------------------------------------------------------------------------- #
class MobileNetClassifier:
    """Implements inference for the Mobilenet TensorRT engine."""
    ...
    def init_model(self):
        ...
        with open(label_map_path, "r") as f:
            ...
            self.classification_labels = [
                labels[key] for key in sorted(labels)
            ]
    ...
    def perform_classification(self, batch, scales=None, min_confidence=0.01):
        ...
        for i in range(self.batch_size):     
            if min_confidence and np.max(probs[i]) < min_confidence:
                classifications.append({
                    'class': 'undefined'
                })

            classifications.append({
                'class': self.classification_labels[np.argmax(probs[i])]
            })
